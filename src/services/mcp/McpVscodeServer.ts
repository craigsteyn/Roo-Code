...
export class McpVscodeServer {
	...
	private setToolRequestHandlers() {
		...
		this.server.setRequestHandler(CallToolRequestSchema, async (request, extra): Promise<CallToolResult> => {
			...
			// instead of using zod we will use json schema validation
		  ...
		  const args = parseResult.data;
			*/
				...
				const cb = tool.callback as ToolCallback
				try {
					result = await Promise.resolve(cb(undefined, extra))
				} catch (error) {
					result = {
						content: [
							{
	...
	registerTool(
		...
		return this._createRegisteredTool(name, description, inputSchema, outputSchema, annotations, cb as ToolCallback)
	}
...
export class VScodeServerTransport implements Transport {
	private _clientTransport: VScodeClientTransport | undefined
	onclose?: () => void
	onerror?: (error: Error) => void
	onmessage?: (message: JSONRPCMessage) => void

	constructor(client: VScodeClientTransport) {
		this._clientTransport = client
	}
	...
	send(message: JSONRPCMessage): Promise<void> {
		if (this._clientTransport?.onmessage) {
...
export class VScodeClientTransport implements Transport {
	private _abortController: AbortController = new AbortController()
	private _server: McpVscodeServer | undefined
	private _serverTransport: VScodeServerTransport | undefined
	onclose?: () => void
	onerror?: (error: Error) => void
	onmessage?: (message: JSONRPCMessage) => void
	...
	async start(): Promise<void> {
		...
		this._server = new McpVscodeServer({
			name: "vscode-server",
			version: "1.0.0",
		})

		// map all tools to an array of strings
		const toolNames: string[] = vscode.lm.tools.map((tool) => tool.name)
		console.log("Available tools:", toolNames)

		for (const tool of vscode.lm.tools) {
			if (tool.name !== "copilot_runVscodeCommand") {
				continue
			}
			...
			this._server.registerTool(
				...
				async (args, extra): Promise<CallToolResult> => {
					// will want to check its still here, but.
					const result: vscode.LanguageModelToolResult = await vscode.lm.invokeTool(name, {
						toolInvocationToken: undefined,
						input: args,
					})
					const content: Array<vscode.LanguageModelTextPart | vscode.LanguageModelPromptTsxPart | unknown> =
						result.content

					// filter out any non-text parts
					const textContent = content.filter(
						(part) => part instanceof vscode.LanguageModelTextPart,
					) as vscode.LanguageModelTextPart[]

					// now we want to map it to the CallToolResult format
					const structuredContent = textContent.map((part) => ({
						type: "text" as const,
						text: part.value,
					}))
		...
		this._serverTransport = new VScodeServerTransport(this)
	...
	send(message: JSONRPCMessage): Promise<void> {
		if (this._serverTransport?.onmessage) {
